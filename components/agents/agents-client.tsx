  ...
  return (
    <div className="max-w-[1200px] mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-gray-900 mb-1">AI Agents</h1>
          <p className="text-sm text-gray-500">{agents.length} agents configured</p>
        </div>
        <Link href="/agents/create" className="px-4 py-2.5 rounded-lg bg-indigo-600 text-white text-sm font-semibold hover:bg-indigo-700 transition-colors shadow-sm">+ New Agent</Link>
      </div>
      <div className="space-y-3">
        {agents.map((agent) => (
          <div key={agent.id} className="bg-white border border-gray-200 rounded-xl p-5 hover:border-indigo-200 hover:shadow-md transition-all shadow-sm">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-3 mt-1.5">
                    <span className="text-[10px] font-mono text-gray-500 bg-gray-100 px-2 py-0.5 rounded">{agent.model}</span>
              ...
              <div className="flex items-center gap-3">
                <Link href="/playground" className="px-3 py-1.5 rounded-lg border border-gray-200 text-[11px] text-gray-500 hover:text-gray-900 hover:border-gray-300 transition-colors">Test</Link>
                <span className={"inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-semibold " + (agent.status === 'ACTIVE' ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-100 text-gray-500')}>
