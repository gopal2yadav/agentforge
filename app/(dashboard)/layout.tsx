      ...
      <div className="flex relative z-10">
        ...
        <aside className="w-56 min-h-screen cosmic-sidebar sticky top-0 flex flex-col py-5 px-3">
          ...
          {[
            ...
            { label: 'Studio', href: '/studio', icon: 'S' },
            { label: 'Agents', href: '/agents', icon: 'A' },
            { label: 'Templates', href: '/templates', icon: 'T' },
            { label: 'Flows', href: '/flows', icon: 'F' },
            { label: 'Automations', href: '/automations', icon: 'Z' },
            { label: 'Playground', href: '/playground', icon: 'P' },
            { label: 'Sandbox', href: '/sandbox', icon: 'C' },
          ].map(item => (
            <a key={item.label} href={item.href} className="flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm text-indigo-200/70 hover:text-white hover:bg-indigo-500/10 transition-all group">
              <span className="w-5 h-5 rounded flex items-center justify-center text-[10px] font-bold bg-indigo-500/10 text-indigo-400/60 group-hover:bg-indigo-500/20 group-hover:text-indigo-300 transition-all">{item.icon}</span>
              {item.label}
            </a>
          ))}
          ...
          <div className="mt-auto px-3 pt-4 border-t border-indigo-500/10">
            <div className="text-[10px] text-indigo-400/40">Nexus v2.8.0</div>
          </div>
        </aside>
