...
export default function StudioPage() {
  ...
  const toggleAgent = (name: string) => setSelectedAgents(prev => prev.includes(name) ? prev.filter(n => n !== name) : [...prev, name]);
  ...
  const handleCreate = async () => {
    ...
    const res = await fetch('/api/crews', {
      ...
      body: JSON.stringify({ name, config: { agents: selectedAgents, process: 'sequential' } })
    });
    ...
    <div className="max-w-[1100px] mx-auto">
      ...
      {showCreate && (
        <div className="bg-white border border-indigo-200 rounded-xl p-5 mb-6 shadow-sm">
          ...
          <div className="mb-4">
            <label className="text-xs font-medium text-gray-500 mb-1 block">Crew Name</label>
            <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Content Team" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:border-indigo-500" />
          </div>
          <div className="mb-4">
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-white border border-gray-200 rounded-xl">
          <div className="text-4xl mb-3">\u{1F46B}</div>
            ...
            <div key={c.id} className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm hover:border-indigo-200 transition-all">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-3">
                  <div className="w-9 h-9 rounded-lg bg-amber-50 flex items-center justify-center text-amber-600 text-sm">\u{1F46B}</div>
              ...
              <div className="text-xs text-gray-400">
                {c.config?.agents?.length || 0} agents \u00B7 {c.config?.process || 'sequential'} process \u00B7 {c.runs || 0} runs
              </div>
            </div>
          ))}
        </div>
