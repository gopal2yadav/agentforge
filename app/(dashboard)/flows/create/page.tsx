        ...
        <div><label className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2 block">Steps ({steps.length})</label>
          <div className="space-y-3">{steps.map((step, i) => (
            <div key={i} className="flex items-center gap-3">
              ...
              <select value={step.agent} onChange={e => updateStep(i, 'agent', e.target.value)} className="bg-white border border-gray-200 rounded-lg px-3 py-2.5 text-sm text-gray-900 w-48"><option value="">Select agent...</option>{AGENTS.map(a => <option key={a} value={a}>{a}</option>)}</select>
