...
const AGENTS = ['Research Agent', 'Writer Agent', 'Code Reviewer', 'Data Analyst', 'Coordinator', 'Summarizer'];
...
export default function FlowCreatePage() {
  ...
  const [steps, setSteps] = useState([{ agent: 'Research Agent', task: '' }]);
  const [saving, setSaving] = useState(false);
  const [saved, setSaved] = useState(false);
  const addStep = () => setSteps(p => [...p, { agent: '', task: '' }]);
  const removeStep = (i) => setSteps(p => p.filter((_, idx) => idx !== i));
  const updateStep = (i, field, val) => setSteps(p => p.map((s, idx) => idx === i ? { ...s, [field]: val } : s));
  const handleSave = () => {
    if (!name.trim()) return;
    setSaving(true);
  ...
  if (saved) return (
    <div className="max-w-[700px] mx-auto text-center py-20">
      ...
      <p className="text-xs text-gray-400">{steps.length} steps &bull; Trigger: {trigger}</p>
    ...
    <div className="max-w-[800px] mx-auto">
      ...
      <div className="space-y-5">
        ...
        <div>
          ...
          <div className="flex gap-2">{TRIGGERS.map(t => (
            <button key={t} onClick={() => setTrigger(t)}
              className={'px-3 py-2 rounded-lg text-xs font-semibold border transition-all ' + (trigger === t ? 'border-indigo-500 bg-indigo-50 text-indigo-700' : 'border-gray-200 text-gray-500 hover:border-gray-300')}>{t}</button>
          ))}</div>
        </div>
        <div>
          <label className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-2 block">Steps ({steps.length})</label>
          <div className="space-y-3">
            {steps.map((step, i) => (
              <div key={i} className="flex items-center gap-3">
                <div className="flex items-center justify-center w-7 h-7 rounded-full bg-indigo-50 text-indigo-600 text-xs font-bold shrink-0">{i + 1}</div>
                <select value={step.agent} onChange={e => updateStep(i, 'agent', e.target.value)}
                  className="bg-white border border-gray-200 rounded-lg px-3 py-2.5 text-sm text-gray-900 w-48">
                  <option value="">Select agent...</option>
                  {AGENTS.map(a => <option key={a} value={a}>{a}</option>)}
                </select>
                <input type="text" value={step.task} onChange={e => updateStep(i, 'task', e.target.value)} placeholder="Task description..."
                  className="flex-1 bg-white border border-gray-200 rounded-lg px-3 py-2.5 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:border-indigo-500" />
                {steps.length > 1 && <button onClick={() => removeStep(i)} className="text-red-400 hover:text-red-600 text-sm">\u2715</button>}
                {i < steps.length - 1 && <div className="absolute left-[13px] mt-12 w-px h-4 bg-indigo-200" />}
              </div>
            ))}
          </div>
          <button onClick={addStep} className="mt-3 px-4 py-2 rounded-lg border border-dashed border-gray-300 text-xs text-gray-500 hover:border-indigo-300 hover:text-indigo-600 transition-colors w-full">+ Add Step</button>
        </div>
